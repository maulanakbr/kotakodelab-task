      ...
      <main className="min-h-screen bg-gray-100">
        <section className="mx-auto min-h-screen max-w-screen-sm bg-white py-10">
          ...
          <div className="mx-3 mt-6 grid grid-cols-1 gap-x-2 gap-y-10 sm:grid-cols-1 lg:grid-cols-2">
            {isLoading && [1, 2, 3, 4].map((_, idx) => <Skeleton className="h-80 rounded-md" key={idx} />)}

            {staffs?.data?.map(({ attributes }, index: number) => (
              <div key={index} className="group relative">
                <div className="aspect-h-1 aspect-w-1 lg:aspect-none min-h-80 w-full overflow-hidden rounded-md bg-gray-200 group-hover:opacity-75 lg:h-80">
                  <img
                ...
                <div className="mt-4">
                  <h3 className="text-sm text-gray-700">
                    <Link
                      href={{ pathname: `/examples/${attributes.id}`, query: { lang: router.query.lang } }}>
                      <a>
